...
div_strain_list <- read.delim("Divergent_and_haplotype/div_strain_list.txt", header=FALSE, stringsAsFactors=FALSE) 
...
if (file.info("Divergent_and_haplotype/all_QTL_bins.bed")$size!=0) {
 ...
 #   write.table(m2, paste0("matrix_for_divergent_heatmap_", QTL_Chrom, QTL_Peak, ".txt"), sep="\t", quote=F, row.names = T, col.names = NA)
    ...
    ref_alt %<>% mutate(geno_group = ifelse(allele==1, "ALT", "REF"))  %>% mutate(geno_group_count = ifelse(allele==1, paste0("ALT(", alt_count[1,2], ")"), paste0("REF(", ref_count[1,2], ")"))) %>% select(-allele)
    ...
    ref_alt_annotation %<>% mutate(is_div = m2[strain, as.character(peak_bin)])
  ...
  #  write.table(ref_alt_annotation, paste0("strain_ref_alt_div_annotation_", QTL_peaks[n, "Chrom"], ".", QTL_peaks[n, "Peak"], ".txt"), sep="\t", quote=F, row.names = F, col.names = T)
    ...
    legend_mapping = data.frame(div_group=c(0, 1, -1), div_label=c("not divergent", "div from N2", "no div data" )) %>% filter(div_group %in% unique(c(m2)))
...
# if (file.info("all_QTL_div.bed")$size!=0) {
    ...
    haplotype_in_QTL_region <- read.delim("Divergent_and_haplotype/haplotype_in_QTL_region.txt", header=FALSE, stringsAsFactors=FALSE) %>% select(-V27)
...
names(haplotype_in_QTL_region) = c("chromosome", "start", "stop", "haplotype", "isotype", "plotpoint", "segment", "color", "color_new", "Chrom", "Region_start", "Region_end")
   ...
   ############ keep relavent columns for plotting
    ...
    plot_df %<>% inner_join(plot_df_sort_order, by="isotype")
