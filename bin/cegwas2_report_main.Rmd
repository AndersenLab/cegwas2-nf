...
ldcalc <- read.table(file = "Genotype_Matrix/LD_between_QTL_regions.tsv", header = T) %>% as.matrix()
...
```{r, results='asis', echo=F, eval=whether_fine_map}
link_list = list()

for (i in seq_len(nrow(QTL_peaks))) { 
    
    region_name = paste0(QTL_peaks[i,"Chrom"], ".", QTL_peaks[i,"Region_start"], "_", QTL_peaks[i,"Region_end"])
