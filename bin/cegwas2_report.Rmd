...
* #### Red/pink dots are significant QTL loci based on defined thresholds. 
    * Red is BF or user supplied cutoff: default BF -log10(p) > 6. 
    * Pink is Eigen cutoff: -log10(0.05/independent_tests) where "independent_tests" is calculated from Eigen Decomposition of SNV correlation matrix, stored in Analysis_Results-date/⁩Genotype_Matrix/total_independent_tests.txt

* #### Light blue regions are QTL intervals that contain markers significantly associate with the trait. This is the region that will be shown in the phenotype-genotype, fine mapping and gene plots below. 
...
for (n in seq_len(nrow(QTL_peaks))) {
    ...
    QTL_peak = paste0(QTL_peaks[n, "Trait"],".",QTL_peaks[n, "Chrom"],".",QTL_peaks[n, "Peak"])
    
    QTL_region = paste0(QTL_peaks[n, "Trait"],".",QTL_peaks[n, "Chrom"],".",QTL_peaks[n, "Region_start"],"-",QTL_peaks[n, "Region_end"])
    ...
    plotly_pxg_plot_list[[n]] <- ggplotly(pxg_plot, tooltip = c("plotly_label", "y")) %>%
        layout(autosize = F, width = 300, height = 400)
    ...
    plotly_fine_plot_list[[n]] <- ggplotly(ld_plot, tooltip = c("plotly_label_ld", "y"))
...
* #### Every dot represents a gene, red color indicates significant based on corresponding test.
* #### SKAT
```{r, echo=FALSE, fig.height=3.5, fig.width=10, out.height=350, out.width=1000}
