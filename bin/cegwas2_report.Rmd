...
* Features for the interactive plots: 
    * Put mouse on plot region to show related underlying data.
...
require(ggbeeswarm) || install.packages("ggbeeswarm")
require(rrBLUP) || install.packages("rrBLUP")
require(plotly) || install.packages("plotly")
require(DT) || install.packages("DT")

# If the libraries are indeed installed for the version of R that nextflow will call, but Rscript still couldn't locate them, uncomment the following lines.
...
if (file.info("Mappings/Data/QTL_peaks.tsv")$size!=0) {
    ...
    if (nrow(QTL_peaks)==0) {
        print("EXIT: No significant QTL region for this trait")
        ...
        if (nrow(QTL_peaks)==0) {
            print("EXIT: Significant QTL regions are too wide for plotting")
            whether_fine_map = FALSE # skip related chunks
    ...
    print("EXIT: No significant peak for this trait")
...
## For each QTL region:
```{r echo=FALSE, eval=whether_fine_map}
...
plotly_pxg_plot_list <- list()
plotly_fine_plot_list <- list()
DT_gene_list <- list()
...
for (n in seq_len(nrow(QTL_peaks))) {
    ...
    QTL_peak = paste0(QTL_peaks[n, "Chrom"],".",QTL_peaks[n, "Peak"])
    
    QTL_region = paste0(QTL_peaks[n, "Chrom"],".",QTL_peaks[n, "Region_start"],"-",QTL_peaks[n, "Region_end"])
    ...
    plotly_pxg_plot_list[[n]] <- ggplotly(pxg_plot, tooltip = c("plotly_label", "y")) %>%
    ...
    plotly_fine_plot_list[[n]] <- ggplotly(ld_plot, tooltip = c("plotly_label_ld", "y")) # %>% toWebGL()
    ...
    # store plot for html output
    plotly_gene_plot_list[[n]] <- ggplotly(gene_plot, tooltip = c("plotly_label_gene_name", "plotly_label_gene_ID", "plotly_label_variant")) %>% toWebGL()
...
for (i in seq_len(nrow(QTL_peaks))) {
    ...
    cat("\n\n")
    cat("\n##Phenotype-genotype plot for the peak SNV marker\n")
    cat("\n\n")
    print(htmltools::tagList(plotly_pxg_plot_list[[i]]))
    cat("\n\n")
    
    cat("\n\n")    
    cat("\n##Fine mapping plot for the QTL region\n")
    cat("\n\n")
    print(htmltools::tagList(plotly_fine_plot_list[[i]]))
    cat("\n\n")
    
    cat("\n\n")
    cat("\n##Gene plot for the QTL region\n")
    cat("\n\n")
